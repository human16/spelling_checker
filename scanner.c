...
int scan_file(char *file) {
        ...
        for (;col < end; col++) {
            if (*(buffer+col) == '\n' || *(buffer+col) == '\0' || *(buffer+col) == ' ') {
                ...
                if (linelen > 0) {
                    ...
                    //search_word is provided with linelen-1 instead of linelen to ommit the '\0'
    ...
    if (col > 0 && linestart < col) {
        ...
        if (word) {
            ...
            word[linelen] = '\0';
            if (DEBUG) printf("[line: %d/%d] String: %s with length: %d\n", linestart, col, word, linelen);
...
int scan_directory(char *directory, char *suffix) {
    ...
    while ((dp = readdir(dirp)) != NULL) {
        ...
        if (!path) {
            if (DEBUG) printf("malloc failed for path\n");
            closedir(dirp);
            return EXIT_FAILURE;
        }
        ...
        if (S_ISREG(file_info.st_mode)) {
            ...
            if (slen == 0 || (namelen >= slen && strcmp(dp->d_name + namelen - slen, suffix) == 0)) {
                if (scan_file(path)) {
                    free(path);
                    closedir(dirp);
                    return EXIT_FAILURE;
